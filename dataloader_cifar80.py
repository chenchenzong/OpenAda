        ...
        if self.mode=='test':
            ...
            if False:
                ...
                if self.noise_mode == "sym":
                    noise_label, self.actual_noise_rate = noisify_dataset(100, train_labels, 'symmetric', self.r, 0.2, 1, False)
                else:
                    noise_label, self.actual_noise_rate = noisify_dataset(100, train_labels, 'asymmetric', self.r, 0.2, 1, False)
    ...
    def run(self,mode,pred=[],prob=[], confident_clean_indexes=[], confident_open_indexes=[]):
        if mode=='warmup':
            ...
            if confident_clean_indexes==[]:
                ...
                count_by_class = [0 for i in range(1+80)]
                for index in confident_indexes:
                    if index in confident_open_indexes:
                        all_dataset.noise_label[index] = 80
                        count_by_class[-1] += 1
                    else:
            ...
            if False:
                ...
                for index in range(len(labeled_dataset.noise_label)):
                    if labeled_dataset.probability[index] > 1.5:
                        labeled_dataset.noise_label[index] = 80
                        count_by_class[-1] += 1
                    else:
